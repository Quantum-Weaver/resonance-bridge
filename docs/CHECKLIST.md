...
| 2026-08-22 | **The media lane lands** (the Tiento lamp 🎻, at KP's ⚛ word) — `media/_common.py` (shared helpers; `env_value` imported from the battlenet room, one house implementation) · `media/screen_lander.py` (KP's movie/TV library: local IMDb re-match 411/447 against the Codex-mirror datasets, Vudu + Movies Anywhere joins, TMDb posters 410 — `--posters tmdb|wiki|none`) · `media/games_lander.py` (PlayStation: 11 played works / 1,326 h + library 349 games / $5,445.25 in cents; IGDB via Twitch 284 exact / 32 best / 26 none; covers 313); both land static JSON + images into `resonance-weaver/static/{movies,games}/` (`npm run movies` · `npm run games`); caches committed beside the scripts (`imdb-match-cache.json` · `poster-cache.json` · `igdb-cache.json` · `review-fuzzy.csv`) plus hand-kept `imdb-overrides.csv` / `igdb-overrides.json`; `.env.example` gains `TWITCH_CLIENT_ID` / `TWITCH_CLIENT_SECRET` · `TMDB_READ_ACCESS_TOKEN` / `TMDB_API_KEY` · `IMDB_DATASETS_DIR` (names only; values by KP's hand); keys grepped clean across landed files, caches, scripts |
